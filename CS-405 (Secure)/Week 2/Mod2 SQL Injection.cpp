...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //Make copy of sql command to ensure the original remains on modified
  ...
  std::size_t culprit = dbCopy.find(" or "); // " or " is the one constant in all the injected sql queries
  if (culprit != string::npos) // if culprit does not equal sstring not found
  { 
      cout << "SQL INJECTION DETECTED!" << endl;
      return false;
  }
