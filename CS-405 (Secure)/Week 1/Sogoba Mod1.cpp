...
bool produces_overflow = false;
bool produces_underflow = false;
...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    T result = start;
    T range_val = 0;
    produces_overflow = false;
   


    for (unsigned long int i = 0; i < steps; ++i)
    {
        //Establish max range of data type unsigned long int
        range_val = std::numeric_limits<T>::max() - result;
        
        //Check that increment is not greater than max range of given data type
        if (increment <= range_val) 
        {
            result += increment;
        }
        else 
        {
            produces_overflow = true;
            break;
        }
    }
...
{
    T result = start; // Set to start's val or max val of given data type
    produces_underflow = false;
    T range_val = 0;

    for (unsigned long int i = 0; i < steps; ++i)
    {
        //Establish max range of data type unsigned long int
        range_val = std::numeric_limits<T>::min() - result;

        //Check that decrement is not greater than max range of given data type
        if (decrement <= range_val) 
        {
            result -= decrement;
        }
        else 
        {
            produces_underflow = true;
            break;
        }
    }
    return result;
